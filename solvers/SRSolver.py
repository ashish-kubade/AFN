...
class SRSolver(BaseSolver):
    def __init__(self, opt):
        ...
        self.LR = self.Tensor()
        self.HR = self.Tensor()
        self.ortho = self.Tensor()
        self.SR = None
        self.save_val_output = opt['save_val_output']
    ...
    def feed_data(self, batch, need_HR=True):
        input_LR = batch['LR']
        input_ortho = batch['ortho']
        self.LR.resize_(input_LR.size()).copy_(input_LR)
        self.ortho.resize_(input_ortho.size()).copy_(input_ortho)

        self.file_name = batch['LR_path']
        # print('Actual input size', self.LR.shape)
        if need_HR:
            target = batch['HR']
            self.HR.resize_(target.size()).copy_(target)
            # print('Actual output size', self.HR.shape)
    ...
    def train_step(self):
        ...
        for i in range(self.split_batch):
            ...
            split_ortho = self.ortho.narrow(0, i*sub_batch_size, sub_batch_size)
            # print('Split LR batch size', split_LR.shape)
            split_HR = self.HR.narrow(0, i*sub_batch_size, sub_batch_size)
            # print('Split HR batch size', split_HR.shape)
            if self.use_cl:
                outputs = self.model(split_LR, split_ortho)
                # print('outputs shape( use_cl case)')
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            # print('self hr size', self.LR.shape)
            if self.self_ensemble and not self.is_train:
                SR = self._forward_x8(self.LR, forward_func)
            else:
                SR = forward_func(self.LR, self.ortho)
            # print('SR shape', len(SR))
            if self.save_val_output:
                np.save(self.file_name[0][:-4] + '_SR', SR[0].detach().float().cpu().numpy()[0,0,:,:])
                np.save(self.file_name[0][:-4] + '_HR', self.HR[0].detach().float().cpu().numpy()[0,:,:])
                np.save(self.file_name[0][:-4] + '_LR', self.LR[0].detach().float().cpu().numpy()[0,:,:])

            if isinstance(SR, list) and self.is_train:
                self.SR = SR[-1]
            else:
                self.SR = SR
        self.model.train()
        if self.is_train:
            pred = self.SR     
            loss_pix = self.criterion_pix(pred, self.HR)
    ...
    def get_current_visual2(self, need_np=True, need_HR=True):
        """
        return LR SR (HR) images
        """
        out_dict = OrderedDict()
#        print('outdir shape',len( self.SR))
        out_dict['LR'] = self.LR.data[0].float().cpu()
        out_SR = []
        for i in self.SR:
           out_SR.append(i.data.float().cpu())
    ...
    def save_current_visual(self, epoch, iter):
        ...
        if epoch % self.save_vis_step == 0:
            ...
            misc.imsave(os.path.join(self.visual_dir, 'epoch_%d_img_%d.png' % (epoch, iter + 1)), visual_images)
